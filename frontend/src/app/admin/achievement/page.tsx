...
export default function AdminAchievement() {
  ...
  useEffect(() => {
    fetch('/api/achievement').then(res => res.json()).then(d => setData(d));
  }, []);
  ...
  async function handleDelete(id: string) {
    ...
    const res = await fetch(`/api/achievement?id=${id}`, { method: 'DELETE' });
    if (res.ok) setData(data.filter((item: any) => item.id !== id));
  }

  return (
    <div className="flex flex-col gap-8">
      <h1 className="text-3xl font-bold">Manage Achievement</h1>
      ...
      <div className="glass p-6 rounded-xl border border-border/50">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-xl">
          <input name="title" required type="text" placeholder="title" className="w-full p-4 rounded-xl bg-background/50 border border-border/50 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all shadow-sm" />\n          <input name="date" required type="text" placeholder="date" className="w-full p-4 rounded-xl bg-background/50 border border-border/50 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all shadow-sm" />\n          <input name="description" required type="text" placeholder="description" className="w-full p-4 rounded-xl bg-background/50 border border-border/50 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all shadow-sm" />
          <button type="submit" className="py-4 bg-primary text-primary-foreground font-black tracking-wider uppercase rounded-xl hover:scale-[1.02] active:scale-95 transition-all shadow-lg hover:shadow-primary/25">Add Achievement</button>
        </form>
      </div>
