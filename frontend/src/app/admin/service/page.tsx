...
export default function AdminService() {
  ...
  useEffect(() => {
    fetch('/api/service').then(res => res.json()).then(d => setData(d));
  }, []);
  ...
  async function handleDelete(id: string) {
    ...
    const res = await fetch(`/api/service?id=${id}`, { method: 'DELETE' });
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="glass p-6 rounded-xl border border-border/50">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-xl">
          ...
          <button type="submit" className="py-4 bg-primary text-primary-foreground font-black tracking-wider uppercase rounded-xl hover:scale-[1.02] active:scale-95 transition-all shadow-lg hover:shadow-primary/25">Add Service</button>
        </form>
      </div>
